...
export default function ToolPageClient({ tool }: ToolPageClientProps) {
  ...
  const colors = toolColors[tool.id] || toolColors['image-compress'];
    ...
    <div className="min-h-screen bg-[#f8f6f3] grid-mesh-bg relative overflow-hidden">
      ...
      <header>
        <div className="px-4 sm:px-6 lg:px-10 py-4 flex items-center justify-between">
          ...
          <Link href="/" className="text-sm text-gray-500 hover:text-gray-800 transition-colors">&larr; All Tools</Link>
        </div>
      </header>
      ...
      <main className="max-w-3xl mx-auto px-4 md:px-6 py-8 md:py-16 relative">
        <nav aria-label="Breadcrumb" className="mb-6">
          <ol className="flex items-center gap-2 text-xs text-gray-400" itemScope itemType="https://schema.org/BreadcrumbList">
            <li itemProp="itemListElement" itemScope itemType="https://schema.org/ListItem">
              <Link href="/" className="hover:text-gray-700 transition-colors" itemProp="item">
            ...
            <li itemProp="itemListElement" itemScope itemType="https://schema.org/ListItem">
              <Link href={`/#${tool.category}-tools`} className="hover:text-gray-700 transition-colors" itemProp="item">
                <span itemProp="name">{categoryLabel} Converters</span>
              </Link>
              <meta itemProp="position" content="2" />
            </li>
            <li aria-hidden="true">&rsaquo;</li>
            <li className="text-gray-600 font-medium" itemProp="itemListElement" itemScope itemType="https://schema.org/ListItem">
        ...
        <div className="mb-6 flex justify-center">
          <div className="rounded-xl overflow-hidden border border-gray-200 bg-white shadow-sm inline-block max-w-full">
            <iframe
              ...
              .map((t) => {
                ...
                return (
                  <Link
                    ...
                    style={{ borderColor: `${tColors.primary}30` }}
                  >
                    <div className="font-medium text-sm text-gray-800 mb-1">{t.name}</div>
                    <div className="text-xs text-gray-400">
                      {t.inputFormats[0].toUpperCase()} &rarr; {t.outputFormat.toUpperCase()}
                    </div>
                  </Link>
                );
              })}
