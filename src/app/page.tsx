...
function ToolCard({ tool, index }: { tool: { id: string; name: string; icon: string; inputFormats: string[]; outputFormat: string; description: string }; index: number }) {
  ...
  return (
    <Link href={`/tools/${tool.id}`}
      ...
      aria-label={`${tool.name} — Convert ${tool.inputFormats[0].toUpperCase()} to ${tool.outputFormat.toUpperCase()}`}
    >
      <div className="flex items-start justify-between mb-4 md:mb-5">
        <div className="w-11 h-11 md:w-12 md:h-12 rounded-xl flex items-center justify-center transition-all duration-300 group-hover:scale-110 group-hover:-rotate-3"
          ...
          {IconComponent ? <IconComponent /> : <span className="text-lg" aria-hidden="true">{tool.icon}</span>}
        </div>
...
function Sparks() {
  ...
  useEffect(() => {
    // Client-only decorative content — safe to initialize state in effect
    // eslint-disable-next-line react-hooks/set-state-in-effect
    setParticles(generateParticles());
  }, []);

  if (!particles) return null;
  return (<div className="spark-container" aria-hidden="true">{particles.map((p, i) => (<div key={i} className="spark" style={p} />))}</div>);
}
...
/* ─── FAQ Data ─── */
const faqData = [
  { question: 'Is FileForge really free?', answer: 'Yes! FileForge is completely free to use. No registration, no subscription, no hidden fees. Ever.' },
  { question: 'Are my files secure?', answer: 'Absolutely. Image conversions happen entirely in your browser — your files never leave your device. Document conversions use enterprise-grade secure APIs that auto-delete files after processing.' },
  { question: 'What is the file size limit?', answer: 'The maximum file size is 50MB per conversion. This ensures fast processing for everyone.' },
  { question: 'What formats are supported?', answer: 'Images: JPG, PNG, WebP, GIF. Documents: Word (DOC/DOCX), Excel (XLS/XLSX), PowerPoint (PPT/PPTX), PDF, plain text (TXT). Video: MP4, AVI, MOV, MKV, WebM. Audio: MP3, WAV, AAC, OGG, FLAC.' },
  { question: 'Do I need to create an account?', answer: 'No! You can use all converters instantly without any registration. Just upload your file and convert.' },
  { question: 'How many conversions can I do?', answer: 'Image converters are unlimited (client-side processing). Document converters include 500 free API credits that never expire.' },
];
...
export default function Home() {
  ...
  const howToSchema = {
    '@context': 'https://schema.org',
    '@type': 'HowTo',
    name: 'How to convert files with FileForge',
    description: 'Convert any file in 4 simple steps using FileForge free online converter.',
    step: [
      {
        '@type': 'HowToStep',
        name: 'Choose Your Converter',
        text: 'Browse our 21+ converters organized by category — images, documents, video, and audio. Pick the one that matches your file format.',
      },
      {
        '@type': 'HowToStep',
        name: 'Upload Your File',
        text: 'Drag and drop your file or click to browse. Max 50MB. Image files stay on your device — zero uploads.',
      },
      {
        '@type': 'HowToStep',
        name: 'Automatic Conversion',
        text: 'Our engine processes images in milliseconds. Documents go through secure APIs and are auto-deleted after conversion.',
      },
      {
        '@type': 'HowToStep',
        name: 'Download Instantly',
        text: 'Your converted file is ready immediately. No registration, no waiting, no hidden fees. Just download and go.',
      },
    ],
  };
    ...
    <div className="min-h-screen bg-forge grid-mesh-bg noise-overlay relative overflow-hidden">
      ...
      <header className="sticky top-0 z-50">
        <div className="px-4 sm:px-6 lg:px-10 py-4 flex items-center justify-between">
          ...
          <nav className="flex items-center gap-5 md:gap-8 text-sm" aria-label="Main navigation">
      ...
      <section className="pt-16 md:pt-28 pb-8 md:pb-12 text-center relative px-4 sm:px-6 lg:px-10">
        <div className="max-w-4xl mx-auto">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white border border-gray-200/60 text-xs font-heading font-semibold text-gray-500 tracking-wider uppercase mb-6 shadow-sm animate-fadeInUp" style={{ opacity: 0, animationDelay: '0.1s', animationFillMode: 'forwards' }}>
            <UI.Sparkle /> Free &bull; No Signup &bull; Private
          </div>
      ...
      <section id="document-tools" className="mb-12 md:mb-20 px-4 sm:px-6 lg:px-10">
        <div className="flex items-center gap-4 mb-4 md:mb-6">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs md:text-sm text-gray-400 truncate">Word, Excel, PPT, PDF &amp; more</p>
          </div>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="section-card p-8 md:p-10">
            <div className="relative">
              <div className="absolute left-[19px] top-0 bottom-0 w-[2px] bg-gradient-to-b from-cyan-200 via-purple-200 to-pink-200 rounded-full" aria-hidden="true" />

              <Step number={1} title="Choose Your Converter" description="Browse our 21+ converters organized by category — images, documents, video, and audio. Pick the one that matches your file format." isLast={false} />
        ...
        <div className="px-4 sm:px-6 lg:px-10 py-10">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4 max-w-6xl mx-auto">
            ...
            <div className="text-center md:text-right">
              <p className="text-xs text-gray-400">&copy; {new Date().getFullYear()} FileForge</p>
              <p className="text-xs text-gray-300 mt-0.5">Built by =[&Ouml;V&Eacute;RK&Iacute;LL]= TECHNOLOGIES</p>
            </div>
          </div>
        </div>
      </footer>
